...
// $flow-disable-line
    ...
    function createMatcher(routes, router) { //创建匹配函数， 返回匹配函数
        var ref = createRouteMap(routes);  //创建路由map
        console.log(ref)
        var pathList = ref.pathList; // ["/aa", "/foo/:id", "/bar"]
        ...
        /*{
            ...
            meta:{}
            name:undefined
            parent: //记录父级 {path: "/foo/:id", regex: /^\/foo\/((?:[^\/]+?))(?:\/(?=$))?$/i, components: {…}, instances: {…}, name: "fo", …}
            path:"/aa"
            props:{}
            redirect: null
            regex: /^\/aa(?:\/(?=$))?$/i
    ...
    var nameMap = ref.nameMap; //存放有name属性的router
